...
const AssetEntryHomeScreen: React.FC = () => {
   ...
   */
    ...
    const getEntriesInDateSections = useMemo(() => {
        return transformEntriesToDateSections(assetEntries)
    }, [assetEntries.values()]);//only run anew if entries in state change. Notice the .values()
    ...
    const displayEntries = () => {
        switch (settings) {
            case DisplayOptions.FLAT_LIST: return <EntryFlatList entries={assetEntries} />
            case DisplayOptions.SPREADSHEET: return <Spreadsheet entries={assetEntries} />
